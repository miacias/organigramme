...
class Query {
    ...
    viewManagers() { // query database for employees table, managers only
    ...
    viewTeams() { // query database for employees table
    ...
    updateRole(roleId, emplId) {
        return this.db.promise().query(`UPDATE employees SET role_id = ${roleId} WHERE id = ${emplId};`);
    }
}
